...
| ClawHub | Published | Latest published version is `douyin-web-control@0.1.2` under `billwang233`. `clawhub inspect douyin-web-control` reports moderation `CLEAN`. |
...
npx clawhub@latest publish skills/douyin-web-control \
  ...
  --changelog "Document isolated douyin-web profiles for concurrent projects."
```
